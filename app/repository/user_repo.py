...
def register_user_repo(user_data:Users,db:Session):
    ...
    # Use .dict() to convert the Pydantic model into a dictionary
    db_user = User(**user_data.dict())  # Unpack the dictionary into the User model constructor
    db.add(db_user)  # Add the User instance to the session
    db.commit()  # Commit the transaction to the database
    db.refresh(db_user)  # Optional: Refresh the instance with the data from the database (e.g., for auto-generated fields)
    return db_user  # Optionally return the created user object
